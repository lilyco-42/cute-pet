...
| 编译好的二进制 / APK / HAR | ⚠️ **先解决 2.2**。二进制里嵌入了那 117 张立绘分层图，这与「把图放在公开 Release 附件里」的法律后果相同，只是多了一步解包 |
| 公开的仓库 + CI artifacts | 同上。注意 CI artifact 是公开的，`main` 分支每次 push 都会产出一份带素材的产物 |

**最小可行路径**：如果你只想先把 CI / 发布链路跑通、这一轮先不动素材，
可以先按现状发布 —— 但请把 Release notes 里的素材来源写清楚，并且心里有数：
2.2 那一项是**未决**的，不是"已确认没问题"。
真正要做一次干干净净的合规公示，来源确认这一步绕不过去。

...
pub fn get_char_pressed() -> Option<char> {
    ...
    // FIFO: chars are pushed in arrival order (lib.rs char_event push); pop()
    // would drain LIFO and reverse multi-char input (e.g. "murasame" -> "emasarum").
    // Drain from the front to preserve typing order.
    if context.chars_pressed_queue.is_empty() {
        None
    } else {
        Some(context.chars_pressed_queue.remove(0))
    }
}
...
pub(crate) fn get_char_pressed_ui() -> Option<char> {
    ...
    if context.chars_pressed_ui_queue.is_empty() {
        None
    } else {
        Some(context.chars_pressed_ui_queue.remove(0))
    }
}
